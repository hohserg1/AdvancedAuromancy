package hohserg.advancedauromancy.utils.endothermic.format
...
object UnpackEvaluations {
  ...
  private def defaultVertexData1(format: VertexFormat) =
    defaultUnpackedQuad(format).toRawArray

  val defaultUnpackedQuad = memoize(defaultUnpackedQuad1)

  private def defaultUnpackedQuad1(format: VertexFormat) = {
    LazyUnpackedQuad(new BakedQuad(new Array[Int](UnpackEvaluations.vertexDataSize(format)), -1, null, Minecraft.getMinecraft.getTextureMapBlocks.getAtlasSprite("minecraft:diamond_block"), false, format))
      .reconstruct(
