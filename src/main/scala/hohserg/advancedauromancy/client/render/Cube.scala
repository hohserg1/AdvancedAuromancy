...
object Cube {
  def apply(x: Float, y: Float, z: Float, w: Float, h: Float, d: Float, textureAtlasSprite: TextureAtlasSprite, color: (Float, Float, Float) = (1, 1, 1), applyDiffuseLighting: Boolean = true): Cube =
    new Cube(x, y, z, w, h, d, 0, 0, 0, 1, 1, 1, textureAtlasSprite, color, applyDiffuseLighting)
...
case class Cube(
                 ...
                 textureAtlasSprite: TextureAtlasSprite, color: (Float, Float, Float), applyDiffuseLighting: Boolean) extends (() => List[UnpackedBakedQuad]) {

  override def apply(): List[UnpackedBakedQuad] = toQuads
